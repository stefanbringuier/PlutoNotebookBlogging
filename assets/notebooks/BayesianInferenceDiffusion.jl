...
#> title = "Bayesian Inference of Transport Properties"
#> date = "2023-05-12"
#> tags = ["Bayesian Methods", "Simulation", "Simulations", "Numerics", "Inference"]
#> description = "Using Turing.jl to infer parameters of equations"
#> license = "CC-BY-4.0"

using Markdown
using InteractiveUtils

# ╔═╡ 938a1657-79fb-4295-992f-019178a7c745
md"""
## Determining Diffusion coefficient
**Author: Stefan Bringuier** 

I've been playing around with the [Turing.jl]() package a bit more and was working through some inverse problem examples. I figured I'd turn the notebook into a post.

The goal is to use Bayesian inference to determine the posterior distribution over the diffusion coefficient for the equation:

$$\frac{\partial c(x,t)}{\partial t} = D \frac{\partial^2 c(x,t)}{\partial x^2}$$

where $c$ is the concentration in 1 spatial dimension. Specifying the initial conditions and boundary conditions:

$$c(x,0) = \exp(-50(x-\frac{1}{2})^2)$$
$$c(0,t) = c(1,t) = 0$$

The PDE is our model for some observation of a physical system, and so we want to update our knowledge of `D` based on how well the model and data agree.

!!! note
	In practice, what you really want is to take some observational data, the model equations, and then find out what the model parameters. You can of course do this using linear regression, but you won't get the uncertainty of the model parameters, only in the model fit. Here I don't have any observational data, so I'll manufacture it by adding noise later on to the numerical solution we get for this PDE.
"""

# ╔═╡ 0d731869-348e-480d-954f-d170d9734ffd
md"""
## Numerical Solution

First, let me solve the PDE numerically. We can do this pretty easily with the package [DifferentialEquations.jl](https://github.com/SciML/DifferentialEquations.jl).

I'll use the true value of the diffusion coefficient as being `0.25`.
!!! note 
    I'm not specifying any units here but I believe you can use Unitful.jl with both DifferentialEquations.jl and Turing.jl

Then I'm going to define two functions, one for the initial conditions, and another for the boundary conditions.
"""

# ╔═╡ 948b8274-3143-46ce-ac80-0fd2f284d3bb
ic(x::AbstractVector) = @. exp(-50*(x-1//2)^2);

# ╔═╡ e218c356-4701-4e52-b57c-30f517a7cc89
bc!(c::AbstractVector) = begin
	c[1] = 0.0
	c[end] = 0.0
	return nothing
end;

# ╔═╡ 487e47da-07c4-44bc-9e00-5dd9dc75dfe9
md"""
With DifferentialEquations.jl you specify your ODE/PDE by defining a function with specific arguments which then gets passed the respective methods from Differential Equations.jl.
"""

# ╔═╡ 67fb5501-7c74-4cdc-b700-557f55ea7a99
begin
	nt = 10
	...
	dx = x.step |> Float64
	cₒ = ic(x);
	function numerical_equation!(dc,c,D,t)
		for i in 2:(nx-1)
			dc[i] = D[1]/dx^2 * (c[i+1] - 2*c[i] + c[i-1])
		end
		bc!(dc)
	end
end;

# ╔═╡ a747875c-87c6-4cb3-817a-3a9bedbae27c
md"""
Now settuing up the ODE problem and numerically solving we get:
"""

# ╔═╡ ac0faef1-7191-48cf-98e8-fac3258120d2
begin
	Dₜ = [0.25]
	prb = ODEProblem(numerical_equation!,cₒ,extrema(t),Dₜ)
	sol = solve(prb, Tsit5(), saveat=t)
end

# ╔═╡ 6be0ffd1-f468-4c1c-8c1a-acae2b992a85
md"""
The solution in the table above has time values in the first column and then all other columns represent the x-location and corresponding concentration value. Below  I  plot the solution to see how the concentration pulse spreads out.
"""

# ╔═╡ b8124a38-1330-4e51-9680-9cd3647dcc95
begin
	cline = palette([:red, :blue,:black], nt)
	plt = plot(;xlabel="X-loc.",ylabel="Concentration")
	for i=1:nt
		plot!(plt,x,sol[:,i],lw=1,
			label="t=$(round((i-1)*Float64(t.step),digits=2))",
			color=cline[i])
	end
	plt
end

# ╔═╡ 2126cfe6-155d-429d-b4b2-ea56856a9146
md"""
## Determining D, Inverse problem

So we now have the solution given D this works when you know D and have a model like our function `numerical_equation`. But what if we only had the model and some data from a black-box function or a physical experiment, how could we then figure out our “best guess” of what D should be.

We can  do this using a Bayesian inference approach. If we assume that we know something about how the value of `D` is distributed (i.e. the probability of `D`) prior to observing any data or selecting a model, then we can update our knowledge of what `D` is by using Bayes Rule to get the new "best guess". In the words of Bayesian methods we calculate the posterior probability density function.

## Using Turing.jl

We can use the probablistic programming library Turing.jl to encode the statement above. Below, I define a function `diffusion_coeff` and then use `~` and then a corresponding distribution for the random variables.

For this problem, I'm going to specify a `Beta` and `InverseGamma` prior for `D` and `σ`, respectively. Here σ is a guess for the noise in the data. For the probability of observing the black-box or experimental data, I use a `Noraml` distribution, which is akin to performing linear-regression of the model+parameters and the data.
"""

# ╔═╡ 47b86a67-97e6-46b4-bb41-61febfb1641a
plot([Beta(2,3),InverseGamma(10,3)],layout=2,legend=false,title=["Prior D" "Prior σ"],size=(675,300))

# ╔═╡ cf48b98b-406f-434c-a2d1-6f47e09efbb9
@model function diffusion_coeff(model,data,time)
	#Priors over diffusion coeff and noise.
	D ~ Beta(2,3)
	σ ~ InverseGamma(10,3)

	# We solve our numerical model with prior on D
	prb = remake(model,p=D)
	sol = solve(prb,Tsit5(),saveat=time)

	#Now we need the likelihood prob of our model
	data ~ arraydist(Normal.(sol,σ))
	return sol
end

# ╔═╡ 414ee459-b959-4bf4-83ae-83da444b9a5f
md"""
Now initializing the probability model we then need to sample the posterior probability, which is just picking out values based on the probabilities we've figured out from our data. We also leverage what are called chains which Now are like paths of these sampled values. We do this in parallel but starting from different spots, just to make sure we're really getting a good look at all possibilities. After letting these chains evolve, we use the values they've come up with to make our best guesses about what's really going on in the model.

Doing this we get the info in the table below.
"""

# ╔═╡ 63d38ef9-12f1-48ba-ba3e-7983c4781d23
begin
	model = diffusion_coeff(prb,sol,t)
	chain = sample(model, NUTS(),1000)
end

# ╔═╡ 800bbc45-3d77-4e6d-b27c-5f0bea11b82f
summarize(chain)

# ╔═╡ 0a8b3730-e83e-475a-a871-c2ef948fb206
md"""
We see that the mean value of `D` recovers the correct value, showing how we can use this for inverse selection of `D` when we don't know it.
"""

# ╔═╡ b0923fdd-45ba-4a81-8b8a-2a5f6c0c91bf
md"""
### Adding noise
The result above isn't very useful because the data provided to the probablistic Turing.jl model was generated from the same problem model that was also feed into it. What we usually have is some observed data from an experiment that has noise, so I'll add some noise to the numerical solution and repeat the sampling.
"""

# ╔═╡ 7021c6b6-91a8-48f9-865a-a1f0d5edd77c
sol_noise = sol .+ randn(nx,nt) * 0.050;

# ╔═╡ 0df6b71e-807c-4739-b707-62780a0fa36a
begin
	model_noise = diffusion_coeff(prb,sol_noise,t)
	chain_noise = sample(model_noise, NUTS(),1000)
end;

# ╔═╡ 2a422965-522b-4bf0-82da-ad7b8c551155
summarize(chain_noise)

# ╔═╡ b5d1029f-55f1-43f3-b906-b9b3560f3453
md"""
It is sometimes important to view the paths of the chains to ensure they "burned-in" so that the sampling is representative. We can also look at the recovered distributions for our variables.
"""

# ╔═╡ e6a348af-0025-445c-a5f0-c32ff4556a9b
plot(chain,size=(675,425),shared_xaxes=true)

# ╔═╡ 2c30ffb2-6a74-4748-9c26-8c090128c1f2
md"""
## 
Whats very nice hear is that even with the addition of noise, we see that the mean value of `D` is very close to the true value and furthermore the value of the noise, `σ` is also close.

## Numerical solutions using samples from posterior
If we want we can observe the effect on the numerical solutions for our `DifferentialEquations.jl` model by sampling from the posterior probablity and then passing those values (i.e., `D`) into the solver. This will let us visually see the spread.
"""

# ╔═╡ 9c73c8a0-0bbb-425a-8bd7-b3ff4e7116b9
posterior_samples = sample(chain_noise[[:D,:σ]], 50; replace=false);

# ╔═╡ 5c4e9085-6c30-40b9-b72f-4f2e05151295
begin
	plt_2 = plot(; legend=false,xlabel="X-loc.",ylabel="Concentration")

	for p in eachrow(Array(posterior_samples))
		sol_p = solve(prb, Tsit5(); p=p, saveat=t)
		for i=1:nt
		    plot!(plt_2,sol_p[:,i]; alpha=0.05, color=cline[i])
		end
	end
	
	for i=1:nt
		scatter!(plt_2,sol_noise[:,i];
			color=cline[i],
			markeralpha=0.5,
			markersize=2.5,
			markerstrokewidth=0,
	...
	annotate!(plt_2,[(25,0.9,text("⚫ Noisy data",9)),
		(25,0.825,text("--- Solutions",9))])
	plt_2
end

# ╔═╡ 4c54d048-98d6-4bbd-90d4-da567d280ef6
md"""
Lets discuss the result above. The markers are the actual solution with D=`0.25` plus some random noise. Now the solution obtained where we sample the from the MCMC the value of D, which is taken from a posterior distribution which contains the noise of the data, is given by the solid colored lines (they should look like bands).

Whats interesting is that using a Bayesian inference we obtain near pefect solutions even in the presence of noise. we can quantify the uncertainty in the solution by looking at the variance of the solutions.

So basically Turing.jl is a simple way to handle the inverse problem of finding the simulation parameters given a model/black-box solver.
"""

# ╔═╡ 0502d6d2-4a50-41c1-8681-800b6448d006
md"""
## Packages
"""

# ╔═╡ b289f3fb-aa5e-4c12-8119-e146f56b2f0e
# ╠═╡ show_logs = false
md"""
$(let
	using Pkg
...
# ╔═╡ b92682ff-facc-4a84-a178-c0a1c58c60d5
# ╠═╡ show_logs = false
begin
...
# ╔═╡ 3437af46-f109-11ed-2fde-c31e457fd8a5
begin
	using Plots, StatsPlots
	plotly();
	using ColorSchemes
	TableOfContents(title="Bayesian Inference of Transport Properties")
end
...
julia_version = "1.9.0"
manifest_format = "2.0"
project_hash = "d3097cb2caa25f3cec9d8fc7a253532e69e4725e"
...
git-tree-sha1 = "adc64f14df848aadf3196572cb8e344f9a19a24b"
